...
export class DataCollectionService {
	...
	async findFilesWithTag(tag: string): Promise<TFile[]> {
		...
		console.log(`Count Novels: Scanning ${files.length} markdown files for tag "${tag}"`);
		...
		for (const file of files) {
			try {
				...
				// タグが存在するかチェック
				if (cache?.tags) {
					hasTag = cache.tags.some(tagRef => 
						tagRef.tag === `#${tag}` || tagRef.tag === tag
					);
					if (hasTag) {
						taggedFiles.push(file);
						console.log(`Count Novels: Found tag "${tag}" in file: ${file.path}`);
					}
				}

				// フロントマターのタグもチェック（重複チェックを追加）
				if (!hasTag && cache?.frontmatter?.tags) {
					const frontmatterTags = Array.isArray(cache.frontmatter.tags) 
						? cache.frontmatter.tags 
						: [cache.frontmatter.tags];
					
					hasTag = frontmatterTags.some((fmTag: string) => 
						fmTag === tag || fmTag === `#${tag}`
					);
					if (hasTag) {
						taggedFiles.push(file);
						console.log(`Count Novels: Found tag "${tag}" in frontmatter of file: ${file.path}`);
					}
				}
			} catch (error) {
				console.warn(`Count Novels: Error checking tags for file ${file.path}:`, error);
			}
		}

		console.log(`Count Novels: Found ${taggedFiles.length} files with tag "${tag}"`);
		return taggedFiles;
	}
	...
	async countCharactersInFile(file: TFile): Promise<number> {
		try {
			...
			console.warn(`Count Novels: Error reading file ${file.path}:`, error);
			return 0;
		}
	}
	...
	async calculateTotalCharacterCount(): Promise<number> {
		...
		try {
			...
			console.log(`Count Novels: Total character count for tag "${tag}": ${totalCount}`);
			return totalCount;
		} catch (error) {
			console.error("Count Novels: Error calculating total character count:", error);
			return 0;
		}
	}
	...
	async collectData(): Promise<void> {
		try {
			...
			console.log(`Count Novels: Previous total: ${previousTotal}, Current total: ${currentTotal}, Difference: ${difference}`);
			...
			console.log(`Count Novels: Data collection completed. Recorded ${difference} characters for ${today}`);
		} catch (error) {
