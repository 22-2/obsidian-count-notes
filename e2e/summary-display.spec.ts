...
It has multiple lines and paragraphs.`,
		...
		await obsidian.save(testFile.path, testFile.content);
		await obsidian.expectExists(testFile.path);
		...
		const viewTestResult = await obsidian.page.evaluate(
			async (pluginId) => {
				const plugin = app.plugins.getPlugin(pluginId) as any;
				if (!plugin) return null;
		...
		// Clean up
		await obsidian.delete(testFile.path);
	});
	...
	test("should handle zero streak when no writing data exists", async ({
		...
		const emptyStreakResult = await obsidian.page.evaluate(
			async (pluginId) => {
				const plugin = app.plugins.getPlugin(pluginId) as any;
				if (!plugin) return null;
				...
				const mockView = {
					...
					formatDateString: function (date: Date) {
	...
	test("should handle broken streak correctly", async ({
		...
		const brokenStreakResult = await obsidian.page.evaluate(
			async (pluginId) => {
				const plugin = app.plugins.getPlugin(pluginId) as any;
				if (!plugin) return null;
...
It should generate some character count data for the chart.`,
		...
		const chartTestResult = await obsidian.page.evaluate(
			async (pluginId) => {
				const plugin = app.plugins.getPlugin(pluginId) as any;
				if (!plugin) return null;
